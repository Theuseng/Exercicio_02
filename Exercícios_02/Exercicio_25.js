...
function somaColunas(matriz) {
    ...
    const vetorSomaColunas = Array(numColunas).fill(0);

    for (let i = 0; i < numColunas; i++) {
        for (let j = 0; j < numLinhas; j++) {
            vetorSomaColunas[i] += matriz[j][i];
        }
    }

    return vetorSomaColunas;
}
...
const vetorSomaColunas = somaColunas(matriz);

console.log("Soma de cada coluna:");
console.log(vetorSomaColunas);
